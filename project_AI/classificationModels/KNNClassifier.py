...
class Method_KNN_Classifier:
    ...
    def train_knn(self, X_train, y_train, n_iter=100, cv=5, random_state=42):
        ...
        print(f"Le modèle KNN a été entraîné avec les meilleurs hyperparamètres: {random_search.best_params_}.")

        # Préparer l'explainer SHAP après l'entraînement
        self.X_train_summary = shap.kmeans(X_train, 30)
        self.explainer = shap.KernelExplainer(self.best_knn.predict_proba, self.X_train_summary)
    ...
    def explain(self, X_instance):
        """
        Explique une instance de données individuelle en utilisant SHAP.
        """
    ...
    def run_KNN_classifier(self, X_train, y_train, X_test, y_test):
        ...
        print('_________________Explicabilité du Modèle KNN avec SHAP_________________')
